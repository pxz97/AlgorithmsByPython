...
DATA_PATH = "./data/ml-latest-small/ratings.csv"  # 源数据集路径
CACHE_DIR = "./data/cache/"  # 数据集缓存路径
...
def load_data(data_path):
    ...
    if os.path.exists(cache_path):  # 判断是否存在缓存文件
        ...
        # 加载数据，只用前三列数据，分别为用户ID，电影ID，用户对电影的对应评分
...
def predict(uid, iid, ratings_matrix, user_similar):
    ...
    for sim_uid, similarity in finally_similar_users.iteritems():
        ...
        # 近邻用户对iid物品的评分
        sim_user_rated_for_item = sim_user_rated_movies[iid]
        # 计算分子的值
        sum_up += similarity * sim_user_rated_for_item
        # 计算分母的值
        sum_down += similarity
...
def _predict_all(uid, item_ids, ratings_matirx, user_similar):
    """
    预测全部评分
    :param uid: 用户id
    :param item_ids: 要预测的物品id列表
    :param ratings_matirx: 用户-物品打分矩阵
    ...
    for iid in item_ids:
        try:
            rating = predict(uid, iid, ratings_matirx, user_similar)
